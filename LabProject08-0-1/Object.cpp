...
#endif
		if (!bDuplicated) //텍스처가 처음이 등장하는 거면 
		{
...
CGameObject *CGameObject::LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, CGameObject *pParent, FILE *pInFile, CShader *pShader)
{
	...
	for ( ; ; )
	{
		...
		if (!strcmp(pstrToken, "<Frame>:"))
		{
			...
			nReads = (UINT)::fread(&nFrame, sizeof(int), 1, pInFile); //프레임번호
			nReads = (UINT)::fread(&nTextures, sizeof(int), 1, pInFile);	// 텍스처개수

			nReads = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pInFile);	//	프레임이름길이
			nReads = (UINT)::fread(pGameObject->m_pstrFrameName, sizeof(char), nStrLength, pInFile); //프레임이름
